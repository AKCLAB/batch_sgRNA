...
def process_file(input_xls, position):
    ...
    #Confer the the nucleotide start in sgRNA
    def transform_sequences(seq_list):
        result = []
        for seq in seq_list:
            if isinstance(seq, str) and len(seq) > 0 and seq[0].upper() in ("T", "C"):
                result.append("G" + seq)
            else:
                result.append(seq)
        return result
    df["target_seq"] = transform_sequences(df["target_seq"])
    ...
    for idx, row in df.iterrows():  
        ...
        if row["position"] == "down" :
            ...
            seq_ref = genome[chrom].seq[hr1:hr1+40]
            s_hr = hr1+1
            f_hr = hr1+41 
        ...
        if row["position"] == "up" :
            ...
            strand_sgrna = dic.get(row["name"]) 
            # extract fragments of 30 nt (adjusting for 0-based python)
            seq_ref = genome[chrom].seq[hr1-40:hr1] 
            s_hr = hr1-40
            f_hr = hr1    
            seq1 = seq_ref    
